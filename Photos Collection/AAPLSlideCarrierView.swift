...
let SLIDE_SHADOW_MARGIN: CGFloat = 0    // margin on each side between the actual slide shape edge and the edge of the SlideCarrier image
let SLIDE_CORNER_RADIUS: CGFloat = 10.0     // corner radius of the slide shape in points
let SLIDE_BORDER_WIDTH: CGFloat = 1.0     // thickness of border when shown, in points
...
class AAPLSlideCarrierView: NSView {
    ...
    private func updateBorderView() {
        ...
        if highlightState == .forSelection {
            ...
            borderColor = NSColor(calibratedRed: 0, green: 0, blue: 0, alpha: 0.5) // Aqua
        ...
        if let borderColor = borderColor {
            if borderView == nil {
                ...
                let shapeBox = NSInsetRect(bounds, (SLIDE_BORDER_WIDTH), (SLIDE_BORDER_WIDTH))
    ...
    private var slideShape: NSBezierPath {
        let bounds = self.bounds
        let shapeBox = NSInsetRect(bounds, 0, 0)
